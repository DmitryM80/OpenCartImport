...
class ModelCatalogKingsilkbtImport extends Model
{

    private $supplierName = 'kingsilk';
    ...
    public function store_db($csv)
    {
        ...
        foreach ($csv as $item) {
            ...
            foreach ($requiredOcFilterOptionFields as $productFilterField) {
                ...
                $currentFilter = ($currentFilter === 'Размер одеяла') ? 'Размер' : $currentFilter;
                $currentFilter = ($currentFilter === 'Материал чехла') ? 'Материал' : $currentFilter;
                $currentFilterValue = $product[$productFilterField];

                if ($currentFilter === 'Размер') {
                    $values = explode('|', $currentFilterValue);
                    foreach ($values as $value) {
                        $this->insertIntoOcFilterOptionTablesWithDelimiter(
                            $currentFilter,
                            $value,
                            $productId,
                            $parentCategoryId);
            ...
            foreach ($requiredOcProductAttributeFields as $productAttributeField) {
                ...
                $currentAttribute = ($currentAttribute === 'Размер одеяла') ? 'Размер' : $currentAttribute;
...
/////////////////
        ...
        if (!empty($ocfOptionId) && !empty($ocfOptionKeywordTranslite)) {
            ...
            $ocfilterOptionValueToProductId = $this->db->query(
                "SELECT `ocfilter_option_value_to_product_id` 
                 ...
                 WHERE `option_id` = {$ocfOptionId} AND `product_id` = {$productId} AND `value_id` = {$ocfOptionValueId}
                 LIMIT 1"
            );
     ...
     * @param $parentCategoryId
     */
    public function insertIntoOcFilterOptionTablesWithDelimiter($currentFilter, $currentFilterValue, $productId, $parentCategoryId)
    {
        $ocfOptionId = $this->selectWhereName(
            'option_id',
            'oc_ocfilter_option_description',
...
/////////////////
    ...
    private function getUpc($sku)
    {
        ...
        return $skuParts[0] . '-' . $skuParts[1] . '-' . $this->supplierName;
    }
    ...
    private function getTranslite($string = null)
    {
        if ($string !== null) {
            ...
            $lat = array(
                'a', 'b', 'v', 'g', 'd', 'e', 'io', 'zh', 'z', 'i', 'j', 'k', 'l', 'm', 'n', 'o', 'p', 'r', 's', 't',
                'u',
