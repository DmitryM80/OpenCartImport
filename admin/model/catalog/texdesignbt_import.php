...
class ModelCatalogTexdesignbtImport extends Model {
...
/////////////////
     ...
     * @return string
     */
    private function getUpc($value)
    {
        $lengthString = strlen(trim($value));

        if ($lengthString <= 3) return $value;
        if ($lengthString === 11) return substr($value,0, 6);
        if ($lengthString === 13) return substr($value,0, 6);
        if ($lengthString === 14) return substr($value,0, 12);
    }
