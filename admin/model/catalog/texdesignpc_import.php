...
class ModelCatalogTexdesignpcImport extends Model {
    ...
    public function store_db($csv)
    {
        ...
        foreach ($csv as $item) {
            ...
            foreach ($requiredOcFilterOptionFields as $productFilterField) {
                ...
                $currentFilter = ($currentFilter === 'Вид застежки') ? 'Тип застежки наволочки' : $currentFilter;
                $currentFilter = ($currentFilter === 'Упаковка') ? 'Упаковка комплекта' : $currentFilter;
                $currentFilter = ($currentFilter === 'Состав материала') ? 'Состав ткани' : $currentFilter;
                $currentFilterValue = $product[$productFilterField];
                $this->insertIntoOcFilterOptionTables($currentFilterValue);
            ...
            foreach ($requiredOcProductAttributeFields as $productAttributeField) {
                ...
                $currentAttribute = ($currentAttribute === 'Вид застежки') ? 'Тип застежки наволочки' : $currentAttribute;
                $currentAttribute = ($currentAttribute === 'Упаковка') ? 'Упаковка комплекта' : $currentAttribute;
                $currentAttribute = ($currentAttribute === 'Состав материала') ? 'Состав ткани' : $currentAttribute;
...
/////////////////
    ...
    private function getUpc($productName)
    {
        ...
        $items = explode(' ', $words[0]);
        $result = [];
        foreach ($items as $item) {
            $result[] = substr($item, 0, 6);
        }
